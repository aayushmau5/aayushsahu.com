...
The idea of using MDX in my blogs came from [this article](https://www.joshwcomeau.com/blog/how-i-built-my-blog/) by [Josh W Comeau](https://www.joshwcomeau.com/) (🙏 thanks, Josh).
...
At first, I was not sure which library to go with. So, I started with `@next/mdx`, but things kinda started going wrong. See how 👇
...
This didn't help because I started getting webpack warnings with remark plugins, and some plugins refused to work, and the internet didn't help 😞
...
One of MDX's prominent feature is to use React components inside your blogs, and It would be a shame if we didn't this feature. So, let's use a React component inside our blog(for the sake of the length of _this_ blog, the component will be a simple one).

```jsx
// utils/ShowContentInside.js
...
Here's an example blog using the component(Make sure to look at [MDX syntax](https://mdxjs.com/docs/using-mdx/) to see how to use components inside MDX).
...
For this, we need to pass our MDX renderer the `ShowContentInside` component(this behavior may differ in other MDX libraries in that you could import component inside your MDX document itself, but `next-mdx-remote` doesn't work like this).
...
First we import our component, and then pass it to `MDXRemote` renderer component through `components` prop as an object. Now, during the rendering of our MDX blog, these components will be used.
...
We have now a working blog page using MDX. However, I did have some concerns after using `next-mdx-remote`. Change are you might have the same concerns. Check them out below.

## Concerns

### Bundle size

When reading up about `next-mdx-remote`, I got to know that all the components are bundled during build time and it is available to every MDX file. Meaning even if some of your blog won't be using a certain component, it will still be available to them. Thus, users have to download those unused components as well.

We can overcomes this issue by using dynamic importing. I found [this thread](https://github.com/hashicorp/next-mdx-enhanced/issues/103) which goes over this.
...
However, I did find [`next-remote-watch`](https://github.com/hashicorp/next-remote-watch) which wasn't really equivalent to HMR, but it gets the job done. I just had to pass the directory(in this case my `posts/` directory) which I wanted it to watch.

I did check [preview mode](https://nextjs.org/docs/advanced-features/preview-mode), which didn't work because it had a 2kb limit, and required more involved work to setup.

# A word about `mdx-bundler`

I have seen some people recommend `mdx-bundler`. It seemed to have all the goodies to use MDX. And it is framework agnostic, so you can use it with anything else.
