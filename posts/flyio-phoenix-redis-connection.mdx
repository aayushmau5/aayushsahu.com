...
[Fly.io](https://fly.io/) is amazing, especially for Phoenix projects. Currently, I have 3 phoenix projects hosted on fly.io(and they have a very generous free tier plan). You can also use fully-managed Redis instance in fly, it's [pretty easy to setup](https://fly.io/docs/reference/redis/).

Recently, I had to connect a personal phoenix project with redis. Aaaand it wasn't easy! I had to do a lot of debugging, had to go through various github issues to figure everything out.
...
At the end, I was able to connect my phoenix app with redis 🎉. I'm putting down how I did it in this blog!

## Things to consider when setting up Redis

When setting up a Redis instance using `flyctl`(Fly's command line tool), make sure that the redis instance is in same organization as the Phoenix app. Note down the private URL of the redis instance(alternatively you can see the private URL using `flyctl redis status <redis-instance-name>`).
...
Notice the `socket_opts: [:inet6]` option, it is necessary to provide this. I'm exactly sure why, it has something to do with fly using IPv6 as their IP address.

Once deployed, Redix will be able to connect to the redis instance. This won't work locally since the URL is private, meaning your application will only be able to access the redis instance when running inside flyio server.
...
It's not good to put redix configuration directly into `start_link`, you would want to put this configuration as an enviroment variable.
