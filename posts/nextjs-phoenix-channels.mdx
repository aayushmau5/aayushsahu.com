---
title: "Connecting my website to Phoenix Server for some real-time data"
...
And I wanted to somehow integrate liveview with my website which is in NextJS. However, I wasn't able to integrate Liveview with NextJS, but Phoenix provides another way for integration with other frontends.
...
Connecting my website with phoenix for real-time user count.

# Try it out yourself

Open up a new tab with the same link, and you should see the metrics change.
...
This will create some files such as: `/lib/<project_name>_web/channels/user_socket.ex` as the socket handler, and a JS file. The JS file contains code through which one can connect to the socket. I won't be needing that since I'm using React.

This command also hints that we need to add some lines in `endpoint.ex` file, which I did. Now I can connect to the socket through the following URL: `http://localhost:4000/socket`(you need a websocket client).

# Connecting to the Phoenix Server

Now that I have a Phoenix server up & running, I needed to connect to the server from my NextJS frontend. For that I needed a "phoenix" package which has does most of the job.
...
Since it's a NextJS project, there's a different method to connect to the server. I chose to connect to the server once my webpage loads. Here's the code:

```ts filename=Socket.tsx
// Socket.tsx
...
This context will have the socket instance through which we connected to the server. This socket will allow us to join channels, and send messages. I'm passing socket as a context so that other components can have access to this socket.
...
Now that the `Socket` component is created, I use this component to wrap the whole application so that each component has access to the socket instance. It will allow children components to connect to their own channels and handle messages.
...
This means whenever a page loads for the first time, it will automatically connect to my phoenix server.
...
Using channels we can subscribe to topics and receive updates. I didn't use `mix phx.gen.channel <channel_name>` to create a channel since it includes some functions I don't need.
...
```elixir filename=user_join_channel.ex
...
I needed to add the following line inside the User socket file:

```elixir filename=user_socket.ex
...
First, I'm using the Socket context to get the socket instance.
...
Now that I have a socket connection as well as joined a channel, I can get the count of users who have joined the same channel topic using Phoenix Presence.
...
Presence tells us about the users who joined a channel or left the channel. I can use this to detect how many users are online at a moment.
...
I use `push` to send the presence data to connected clients. The code for server is done.

## Showing presence count in the frontend

The code of presence is done in the server, now its a matter of showing the presence count. I made changes in the same place where I join a channel.
...
and then, instantiated presence, providing the channel connection.

```tsx
let presence = new Presence(phoenixChannel);
```

The presence object has various methods, one of them is `onSync` which gets called whenever the presence data changes(user either leaves or joins).

I then use `presence.list` to get the list of users(or socket) that are online. The length of the list is the number of users who are online. I store this count in the state, and show it to the user.
...
Connecting my website to a real-time phoenix server opens up a lot of opportunities such as Like counter, real-time commenting system, etc. Can't wait to explore these ideas!
